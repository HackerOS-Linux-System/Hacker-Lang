...
fn parse_hacker_file(
    path: &Path,
    verbose: bool,
    bytes_mode: bool,
    optimize: bool,  // Nowy parametr dla optymalizacji
) -> io::Result<(
    Vec<String>,
    Vec<String>,
    Vec<(String, String)>,
    ...
    for line in reader.lines() {
        ...
        if line.starts_with("//") {
            ...
            if lib.is_empty() {
                ...
                let lib_bin_path = expand_home(&format!("{}/libs/{}", HACKER_DIR, lib));
                if Path::new(&lib_hacker_path).exists() {
                    includes.push(lib.clone());
                    let (sub_deps, sub_libs, sub_vars, sub_cmds, sub_includes, sub_binaries, sub_local_vars, sub_errors, sub_config, sub_plugins, sub_functions) = parse_hacker_file(Path::new(&lib_hacker_path), verbose, bytes_mode, optimize)?;
    ...
    // Optymalizacja: Usuń duplikaty deps i libs jeśli optimize
    if optimize {
        deps.sort();
        deps.dedup();
        libs.sort();
...
fn main() -> io::Result<()> {
    ...
    let mut optimize = false;  // Nowa flaga dla optymalizacji
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--optimize" => optimize = true,  // Nowa opcja
            _ => {
    ...
    if input_path_str.is_empty() || output_path_str.is_empty() {
        eprintln!("Usage: hacker-compiler <input.hacker> <output> [--verbose] [--bytes] [--optimize]");
        process::exit(1);
    }
    let input_path = Path::new(&input_path_str);
    let output_path = Path::new(&output_path_str);
    let (deps, _libs, vars, cmds, _includes, binaries, local_vars, errors, _config, plugins, _functions) = parse_hacker_file(input_path, verbose, bytes_mode, optimize)?;
    if !errors.is_empty() {
        for err in errors {
            eprintln!("{}", err);
        }
    ...
    // Optymalizacja: Usuń duplikaty cmds jeśli optimize
    let substituted_cmds = if optimize {
        let mut unique_cmds = std::collections::HashSet::new();
        let mut optimized_cmds = Vec::new();
        for cmd in cmds {
            if unique_cmds.insert(cmd.clone()) {
                optimized_cmds.push(cmd);
            }
        }
    ...
    for (i, bin_path) in binaries.iter().enumerate() {
        let mut bin_data = fs::read(bin_path)?;
        let data_name = format!("bin_lib_{i}");

        // Optymalizacja: Kompresja danych binarnych jeśli optimize
        let encoded = if optimize {
            let mut encoder = GzEncoder::new(Vec::new(), Compression::best());
            encoder.write_all(&bin_data)?;
            bin_data = encoder.finish()?;
            let base64_encoded = BASE64_STANDARD.encode(&bin_data);
            format!("echo '{}' | base64 -d | gunzip > /tmp/{} && chmod +x /tmp/{} && /tmp/{}", base64_encoded, data_name, data_name, data_name)
        } else {
            let base64_encoded = BASE64_STANDARD.encode(&bin_data);
            format!("echo '{}' | base64 -d > /tmp/{} && chmod +x /tmp/{} && /tmp/{}", base64_encoded, data_name, data_name, data_name)
        };

        bin_extract_cmds.push(encoded);
    }
    final_cmds.extend(bin_extract_cmds);

    // Ustawienia Cranelift z optymalizacjami
    let mut flag_builder = settings::builder();
    flag_builder.set("opt_level", if optimize { "speed_and_size" } else { "speed" }).unwrap();
    flag_builder.set("enable_verifier", if verbose { "true" } else { "false" }).unwrap();  // Wyłącz verifier dla szybszej kompilacji
    ...
    // Declare external functions - Optymalizacja: Minimalizuj importy jeśli możliwe
    ...
    // Embed cmd strings - Optymalizacja: Kompresja stringów cmd jeśli optimize
    let mut cmd_data_ids = Vec::new();
    for (i, cmd) in final_cmds.iter().enumerate() {
        let data_name = format!("cmd_{i}");
        let data_id = module.declare_data(&data_name, Linkage::Local, true, false).unwrap();
        let mut data_ctx = DataDescription::new();
        let mut bytes = if optimize {
            let mut encoder = GzEncoder::new(Vec::new(), Compression::best());
        ...
        // Jeśli skompresowane, dodaj dekompresję w cmd, ale ponieważ używamy system, musielibyśmy zmodyfikować cmd do dekompresji
        // Zamiast tego, dla skompresowanych cmd, zmień final_cmds na dekompresujące, ale to komplikuje - pomiń dla cmd, kompresuj tylko binarki
    }
    ...
    // Embed binary libs data - Już skompresowane powyżej
    for (i, bin_path) in binaries.iter().enumerate() {
    ...
    if bytes_mode {
        for (i, bin_path) in binaries.iter().enumerate() {
            let mut bin_data = fs::read(bin_path)?;
            let extract_str = if optimize {
                let mut encoder = GzEncoder::new(Vec::new(), Compression::best());
                encoder.write_all(&bin_data)?;
                bin_data = encoder.finish()?;
                let encoded = BASE64_STANDARD.encode(&bin_data);
                format!("echo '{}' | base64 -d | gunzip > /tmp/bin_lib_{i} && chmod +x /tmp/bin_lib_{i} && /tmp/bin_lib_{i}", encoded)
            } else {
                let encoded = BASE64_STANDARD.encode(&bin_data);
                format!("echo '{}' | base64 -d > /tmp/bin_lib_{i} && chmod +x /tmp/bin_lib_{i} && /tmp/bin_lib_{i}", encoded)
            };
            let data_name = format!("bin_lib_data_{i}");
            let data_id = module.declare_data(&data_name, Linkage::Local, true, false).unwrap();
            let mut data_ctx = DataDescription::new();
    ...
    // Optymalizacja linkowania: Użyj flag GCC dla mniejszych i szybszych binarek
    let gcc_flags = if optimize {
        "-O3 -s -flto"  // O3 dla prędkości, -s strip, -flto link-time opt
    } else {
        "-O2"
    };
    let status = Exec::shell(format!("gcc {} -o {} {}", gcc_flags, output_path.display(), temp_obj_path.display()))
    .join()
    ...
    if optimize {
        // Dodatkowa optymalizacja: Strip binarki jeśli nie zrobione
        let _ = Exec::shell(format!("strip {}", output_path.display())).join();
    }
