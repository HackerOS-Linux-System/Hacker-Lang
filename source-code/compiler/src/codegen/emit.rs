...
/// Wygeneruj plik obiektowy ELF z HlProgram uzywajac Cranelift
pub fn emit_object(prog: &HlProgram, output: &Path, opt: OptLevel, verbose: bool) -> Result<()> {
    // ── ISA: x86_64 lub natywny ───────────────────────────────────────────────
    ...
    // ── Zadeklaruj zewnetrzne funkcje runtime C ──────────────────────────────
    let rt = RuntimeSymbols::declare(&mut module)?;

    // ── Wbuduj string pool jako dane statyczne ───────────────────────────────
    let string_data_ids = emit_string_pool(prog, &mut module)?;

    // ── Emituj kazda funkcje HL ──────────────────────────────────────────────
    ...
    // ── Emituj __hl_entry (wywoluje __hl_main + deps check) ─────────────────
    ...
    if verbose {
        eprintln!("  Object: {} bytes", bytes.len());
    }
    Ok(())
}
...
impl RuntimeSymbols {
    fn declare(module: &mut ObjectModule) -> Result<Self> {
        // i8* ptr (pointer = i64 on x86_64)
        let ptr  = types::I64;
        let i32t = types::I32;

        // hl_print(const char* msg) -> void
        let hl_print = decl_fn(module, "hl_print", &[ptr], &[])?;

        // hl_print_interp(const char* template) -> void
        let hl_print_interp = decl_fn(module, "hl_print_interp", &[ptr], &[])?;

        // hl_run_cmd(const char* cmd, int mode) -> int
        let hl_run_cmd = decl_fn(module, "hl_run_cmd", &[ptr, i32t], &[i32t])?;

        // hl_set_var(const char* name, const char* value) -> void
        let hl_set_var = decl_fn(module, "hl_set_var", &[ptr, ptr], &[])?;

        // hl_set_var_interp(const char* name, const char* tmpl) -> void
        let hl_set_var_interp = decl_fn(module, "hl_set_var_interp", &[ptr, ptr], &[])?;

        // hl_export_var(const char* name, const char* value) -> void
        let hl_export_var = decl_fn(module, "hl_export_var", &[ptr, ptr], &[])?;

        // hl_export_var_interp(const char* name, const char* tmpl) -> void
        let hl_export_var_interp = decl_fn(module, "hl_export_var_interp", &[ptr, ptr], &[])?;

        // hl_export_list(const char* name, const char** items, int count) -> void
        let hl_export_list = decl_fn(module, "hl_export_list", &[ptr, ptr, i32t], &[])?;

        // hl_quick(const char* name, const char* args) -> int
        let hl_quick = decl_fn(module, "hl_quick", &[ptr, ptr], &[i32t])?;

        // hl_dep_check(const char* name) -> int
        let hl_dep_check = decl_fn(module, "hl_dep_check", &[ptr], &[i32t])?;

        // hl_get_last_exit() -> int
        let hl_get_last_exit = decl_fn(module, "hl_get_last_exit", &[], &[i32t])?;

        Ok(Self {
            hl_print, hl_print_interp, hl_run_cmd, hl_set_var, hl_set_var_interp,
            hl_export_var, hl_export_var_interp, hl_export_list, hl_quick, hl_dep_check, hl_get_last_exit,
        })
    }
}
...
fn emit_function(
    ...
    prog:          &HlProgram,
    rt:            &RuntimeSymbols,
    string_ids:    &[cranelift_module::DataId],
    module:        &mut ObjectModule,
    _verbose:      bool,
) -> Result<cranelift_module::FuncId> {
    let sig = module.make_signature();
    // Kazda funkcja HL: void fn()
    // __hl_main: void fn()
...
fn emit_instr(
    ...
    match instr {
        ...
        HlInstr::ExportList { name_idx, items } => {
            // Buduj tablice wskaznikow do stringow na stosie
            // Uproszczone: przekaz pierwszy element i liczbe
            let name = get_str_ptr(*name_idx, string_ids, module, builder)?;
            // Dla listy: przekaz pierwszy element jako wartosc (uproszczone)
            // Pelna implementacja wymaga alokacji tablicy — tutaj robimy join ':'
            let joined = items.iter()
                .filter_map(|&i| prog.string_pool.get(i as usize))
                .cloned()
                .collect::<Vec<_>>()
                .join(":");
            // To bedzie wkompilowane jako string statyczny
            // UWAGA: w pelnej implementacji nalezy intern joined do pool
            // Tutaj uzyj hl_export_var z juz zlaczona wartoscia
            let _ = joined; // uzyte ponizej conceptually
            let fref = module.declare_func_in_func(rt.hl_export_var, builder.func);
        ...
        HlInstr::CallFunc { func_idx } => {
            // func_idx to name_idx w string pool — wywolujemy po nazwie przez runtime
            // W pelnym kompilatorze: direct call do funkcji w tym samym module
            // Tutaj uzyj hl_print jako placeholder sygnalizujacy brakujaca funkcje
            let _name = get_str_ptr(*func_idx, string_ids, module, builder)?;
            // TODO: resolve function by name at link time
        }

        HlInstr::CondOk { body } => {
            // Pobierz last exit code
...
fn emit_entry(
    _deps:       &[String],
    _pool:       &[String],
    _string_ids: &[cranelift_module::DataId],
    _rt:         &RuntimeSymbols,
    module:      &mut ObjectModule,
    func_ids:    &[(String, cranelift_module::FuncId)],
) -> Result<()> {
    // Emituj `main` (C ABI) ktory inicjalizuje runtime i wywoluje __hl_main
    let mut sig = module.make_signature();
