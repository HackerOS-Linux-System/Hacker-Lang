...
pub enum IrOp {
    ...
    // Program poza :: blokami nadal używa libgc.a.
    // Wewnątrz ArenaEnter..ArenaExit — tylko libaa.a (bump ptr, zero GC).
...
impl IrBuilder {
        ...
        for name in names {
            ...
            let arena_spec = if *is_arena {
                nodes.iter().find_map(|n| {
                    if let CommandType::ArenaDef { size_spec, .. } = &n.content { Some(size_spec.clone()) } else { None }
                }).or(Some("1mb".to_string()))
            } else { None };

            let mut ops = Vec::new();
            if *is_arena {
                let spec = arena_spec.as_deref().unwrap_or("1mb");
                ops.push(IrOp::ArenaEnter { name: name.clone(), size_spec: spec.to_string(), size_bytes: parse_size_spec(spec) });
            }
            ...
            module.functions.push(IrFunction { name: name.clone(), is_unsafe: false, is_arena: *is_arena, arena_spec, type_sig: sig.clone(), ops });
        }
        ...
        while i < nodes.len() {
            ...
            match &node.content {
                CommandType::If { cond, cmd } => {
                    ...
                    loop {
                        ...
                        match &nodes[i].content {
                            CommandType::Elif { cond, cmd } => { branches.push(IrBranch { cond: Some(cond.clone()), body: vec![self.lower_single_cmd(cmd, nodes[i].is_sudo)], sudo: nodes[i].is_sudo }); i += 1; }
                            CommandType::Else { cmd }       => { branches.push(IrBranch { cond: None, body: vec![self.lower_single_cmd(cmd, nodes[i].is_sudo)], sudo: nodes[i].is_sudo }); i += 1; break; }
                            _ => break,
                        }
                    }
                    ops.push(IrOp::IfChain { branches }); continue;
                }
                CommandType::Match { cond } => {
                    let mut arms = Vec::new(); i += 1;
                    while i < nodes.len() {
                        if let CommandType::MatchArm { val, cmd } = &nodes[i].content {
                            arms.push(IrArm { val: if val == "_" { None } else { Some(val.clone()) }, cmd: cmd.clone() }); i += 1;
                        } else { break; }
                    }
                    ops.push(IrOp::MatchCase { cond: cond.clone(), arms }); continue;
                }
                CommandType::MatchArm { .. } => { i += 1; continue; }
                CommandType::Pipe(steps) => {
                    let ir = steps.iter().map(|s| { let t = s.trim(); let is_hl = is_hl_call(t); IrPipeStep { cmd: if is_hl { t.trim_start_matches('.').to_string() } else { t.to_string() }, is_hl } }).collect();
                    ops.push(IrOp::Pipe { steps: ir }); i += 1; continue;
                }
                CommandType::ArenaDef { name, size_spec, body } => {
                    let bytes = parse_size_spec(size_spec);
                    ops.push(IrOp::ArenaEnter { name: name.clone(), size_spec: size_spec.clone(), size_bytes: bytes });
                ...
                CommandType::DoBlock { key, body } => {
                    ...
                    ops.push(IrOp::DoBlock { key: key.clone(), body: inner }); i += 1; continue;
                }
                CommandType::TestBlock { desc, body } => {
                    let inner = self.lower_body(body);
                    ops.push(IrOp::TestBlock { desc: desc.clone(), body: inner }); i += 1; continue;
                }
    ...
    fn lower_single_cmd(&self, cmd: &str, sudo: bool) -> IrOp {
        ...
        if is_hl_call(t) {
            ...
            IrOp::CallHL { name: parts[0].trim_start_matches('.').to_string(), args: parts.get(1).filter(|a| !a.is_empty()).map(|a| a.to_string()) }
        } else {
    ...
    fn lower_node(&mut self, node: &ProgramNode, ops: &mut Vec<IrOp>) {
        ...
        match &node.content {
            ...
            CommandType::Out(val) => {
                ...
                } else { ops.push(IrOp::SetOut { val: val.clone() }); }
            }
            ...
            CommandType::Loop { count, cmd } => {
                if *count == 0 {
                    if let Some(nf) = parse_numfor_payload(cmd) {
                        ...
                        ops.push(IrOp::NumFor { var, start: IrOperand::Lit(IrLit::I64(nf.start)), end: IrOperand::Lit(IrLit::I64(nf.end)), step: IrOperand::Lit(IrLit::I64(nf.step)), env_key: nf.var.clone(), body });
                        return;
                    }
                }
            ...
            CommandType::Call { path, args } => {
                let t = path.trim();
                if is_hl_call(t) {
                    ops.push(IrOp::CallHL { name: t.trim_start_matches('.').to_string(), args: if args.is_empty() { None } else { Some(args.clone()) } });
                } else {
            ...
            CommandType::ModuleCall { path, args } => {
                ...
                ops.push(IrOp::CallModule { module: module.to_string(), method: method.to_string(), args: if args.is_empty() { None } else { Some(args.clone()) } });
            }
            ...
            // Arena operacje — pojawiają się WEWNĄTRZ :: bloków
            ...
            // Kolekcje
            CommandType::CollectionMut { var, method, args } => { ops.push(IrOp::CollectionMut { var: var.clone(), method: method.clone(), args: args.clone() }); }
            // Error
            ...
            // Misc
            CommandType::Assert { cond, msg } => { ops.push(IrOp::Assert { cond: cond.clone(), msg: msg.as_deref().unwrap_or("Assertion failed").to_string() }); }
            CommandType::Plugin { name, args, is_super } => {
                let root = get_plugins_root();
                let bin  = root.join(name);
                let hl   = root.join(format!("{}.hl", name));
                let path = if bin.exists() { bin.to_str().unwrap_or(name).to_string() }
                else if hl.exists() { format!("hl {}", hl.display()) }
                else { if self.verbose { eprintln!("{} Plugin '{}' nie znaleziony", "[!]".yellow(), name); } return; };
            ...
            CommandType::Lock { key, val } => {
                ...
                let var = IrVar::named(key); self.reg(key, var.clone(), IrType::Ptr);
                ops.push(IrOp::GcAlloc { var, size });
            }
            CommandType::Unlock { key } => { ops.push(IrOp::Unlock { key: key.clone() }); ops.push(IrOp::GcFree); }
            CommandType::End { code } => { ops.push(IrOp::GcFull); ops.push(IrOp::Exit { code: *code }); }
            CommandType::Extern { path, static_link } => { ops.push(IrOp::Comment { text: format!("extern {} static={}", path, static_link) }); }
            CommandType::Enum      { name, variants }       => { ops.push(IrOp::Comment { text: format!("enum {} [{}]", name, variants.join(", ")) }); }
            CommandType::Struct    { name, fields }         => { ops.push(IrOp::Comment { text: format!("struct {} [{}]", name, fields.iter().map(|(k,t)| format!("{}:{}", k, t)).collect::<Vec<_>>().join(", ")) }); }
            CommandType::AdtDef    { name, variants }       => { ops.push(IrOp::Comment { text: format!("adt {} ({} variants)", name, variants.len()) }); }
            CommandType::Interface { name, methods }        => { ops.push(IrOp::Comment { text: format!("interface {} [{}]", name, methods.join(", ")) }); }
            CommandType::ImplDef   { class, interface }     => { ops.push(IrOp::Comment { text: format!("impl {} for {}", class, interface) }); }
            CommandType::FuncDefGeneric { name, sig }       => { ops.push(IrOp::Comment { text: format!("generic fn {} sig={}", name, sig) }); }
            CommandType::Import    { resource, namespace }  => { ops.push(IrOp::Comment { text: format!("import {:?}{}", resource, namespace.as_deref().map(|n| format!(" in {}", n)).unwrap_or_default()) }); }
            CommandType::SysDep(dep)                        => { ops.push(IrOp::Comment { text: format!("sysdep {}", dep) }); }
            ...
            NumericPayload::Typed { type_, expr } => {
                ...
                match type_.as_str() {
                    "float" => {
                        ...
                        let final_op = if expr_ty != IrType::F64 { let c = ir_uid("cvtf"); self.var_types.insert(c.clone(), IrType::F64); ops.push(IrOp::IntToFloat { dst: c.clone(), src: operand }); IrOperand::Var(c) } else { operand };
                    ...
                    _ => {
                        ...
                        let final_op = if expr_ty == IrType::F64 { let c = ir_uid("cvti"); self.var_types.insert(c.clone(), IrType::I64); ops.push(IrOp::FloatToInt { dst: c.clone(), src: operand }); IrOperand::Var(c) } else { operand };
    ...
    fn lower_expr_flat(&mut self, expr: &HlExpr, _p: &mut Vec<IrOp>) -> (Vec<IrOp>, (IrType, IrOperand)) {
        match expr {
            ...
            HlExpr::Var(name) => {
                let (var, ty) = self.vars.get(name.as_str()).cloned().unwrap_or_else(|| { let v = IrVar::named(name); self.var_types.insert(v.clone(), IrType::I64); (v, IrType::I64) });
                let tmp = ir_uid("lv"); self.var_types.insert(tmp.clone(), ty);
                let load = if ty == IrType::F64 { IrOp::LoadF64 { dst: tmp.clone(), src: var } } else { IrOp::LoadI64 { dst: tmp.clone(), src: var } };
            ...
            HlExpr::Neg(inner) => {
                ...
                let dst = ir_uid("neg"); self.var_types.insert(dst.clone(), ty);
                sub_ops.push(if ty == IrType::F64 { IrOp::NegF64 { dst: dst.clone(), src: op } } else { IrOp::NegI64 { dst: dst.clone(), src: op } });
    ...
    fn lower_binop_flat(&mut self, op: IrBinOp, a: &HlExpr, b: &HlExpr) -> (Vec<IrOp>, (IrType, IrOperand)) {
        ...
        let dst = ir_uid(if is_f { "f64" } else { "i64" }); self.var_types.insert(dst.clone(), ty);
        let oa2 = if is_f && ta != IrType::F64 { let c = ir_uid("cvt"); self.var_types.insert(c.clone(), IrType::F64); ops_a.push(IrOp::IntToFloat { dst: c.clone(), src: oa }); IrOperand::Var(c) } else { oa };
        let ob2 = if is_f && tb != IrType::F64 { let c = ir_uid("cvt"); self.var_types.insert(c.clone(), IrType::F64); ops_a.push(IrOp::IntToFloat { dst: c.clone(), src: ob }); IrOperand::Var(c) } else { ob };
        ops_a.push(if is_f { IrOp::BinF64 { dst: dst.clone(), lhs: oa2, op, rhs: ob2 } } else { IrOp::BinI64 { dst: dst.clone(), lhs: oa2, op, rhs: ob2 } });
    ...
    fn lower_cmp_flat(&mut self, op: IrCmpOp, a: &HlExpr, b: &HlExpr) -> (Vec<IrOp>, (IrType, IrOperand)) {
        ...
        ops_a.extend(ops_b);
        let is_f = ta == IrType::F64 || tb == IrType::F64;
        let dst = ir_uid("cmp"); self.var_types.insert(dst.clone(), IrType::Bool);
        let oa2 = if is_f && ta != IrType::F64 { let c = ir_uid("cvt"); self.var_types.insert(c.clone(), IrType::F64); ops_a.push(IrOp::IntToFloat { dst: c.clone(), src: oa }); IrOperand::Var(c) } else { oa };
        let ob2 = if is_f && tb != IrType::F64 { let c = ir_uid("cvt"); self.var_types.insert(c.clone(), IrType::F64); ops_a.push(IrOp::IntToFloat { dst: c.clone(), src: ob }); IrOperand::Var(c) } else { ob };
        ops_a.push(if is_f { IrOp::CmpF64 { dst: dst.clone(), lhs: oa2, op, rhs: ob2 } } else { IrOp::CmpI64 { dst: dst.clone(), lhs: oa2, op, rhs: ob2 } });
...
impl IrPrinter {
    ...
    pub fn dump(&mut self, module: &IrModule) -> String {
        ...
        self.line("; ──────────────────────────────────────────────────────────");
        self.out.push('\n');
        for (p, s) in &module.extern_libs { self.line(format!("{} {:?}", if *s {"LINK_STATIC"} else {"LINK_DYN   "}, p)); }
        ...
        for i in fi {
            ...
            self.indent -= 1;
            self.line("END_FN"); self.out.push('\n');
        }
        self.line("MAIN"); self.indent += 1;
        for op in &module.main.clone() { self.emit_op(op); }
        self.indent -= 1; self.line("END_MAIN");
        self.out.clone()
    }
    ...
    fn emit_op(&mut self, op: &IrOp) {
        match op {
            ...
            IrOp::CallHL     { name, args }         => match args { Some(a) => self.line(format!("CALL_HL      {:?}  args={:?}", name, a)), None => self.line(format!("CALL_HL      {:?}", name)) },
            IrOp::CallModule { module, method, args} => match args { Some(a) => self.line(format!("CALL_MOD     {}.{}  args={:?}", module, method, a)), None => self.line(format!("CALL_MOD     {}.{}", module, method)) },
            ...
            IrOp::IfChain { branches } => {
                for (bi, br) in branches.iter().enumerate() {
                    match &br.cond { Some(c) if bi == 0 => self.line(format!("IF           {:?}", c)), Some(c) => self.line(format!("ELIF         {:?}", c)), None => self.line("ELSE") }
            ...
            IrOp::MatchCase { cond, arms } => {
                ...
                self.indent += 1;
                for arm in arms { match &arm.val { Some(v) => self.line(format!("ARM          {:?}  {:?}", v, arm.cmd)), None => self.line(format!("ARM_DEFAULT  {:?}", arm.cmd)) } }
                self.indent -= 1; self.line("END_MATCH");
            }
            IrOp::Pipe { steps } => {
                self.line(format!("PIPE         {}", steps.iter().map(|s| if s.is_hl { format!("[HL]{}", s.cmd) } else { s.cmd.clone() }).collect::<Vec<_>>().join(" | ")));
            }
            ...
            // Arena — wyraźna granica GC/Arena w dumpe
