...
module Hl
  ...
  def self.ensure_hacker_dir
    ...
    puts "Failed to create hacker directory: #{error.message}".colorize(:red)
    exit(1)
  end

  def self.display_welcome
    puts "Welcome to Hacker Lang CLI v#{VERSION}".colorize.mode(:bold).back(:magenta).fore(:white)
    puts "Simplified tool for running and compiling .hacker scripts".colorize(:light_gray)
    puts "Type 'hl help' for available commands.".colorize(:white)
    help_command(false)
  end
  ...
  def self.run_command(file : String, verbose : Bool) : Bool
    ...
    unless File.exists?(full_runtime_path)
      puts "Hacker runtime not found at #{full_runtime_path}. Please install the Hacker Lang tools.".colorize(:red)
      return false
    end
    ...
    puts "Executing script: #{file}#{verbose ? " (verbose mode)" : ""}".colorize(:cyan)
    ...
    begin
      ...
      if status.success?
        ...
        puts "Execution failed with exit code #{status.exit_code}".colorize(:red)
        return false
      end
    rescue error
      puts "Error executing command: #{error.message}".colorize(:red)
      return false
    end
  end
  ...
  def self.compile_command(file : String, output : String, verbose : Bool) : Bool
    ...
    unless File.exists?(full_compiler_path)
      puts "Hacker compiler not found at #{full_compiler_path}. Please install the Hacker Lang tools.".colorize(:red)
      return false
    end
    ...
    puts "Compiling script: #{file} to #{output}#{verbose ? " (verbose mode)" : ""}".colorize(:cyan)

    begin
      process = Process.new(full_compiler_path, args, output: Process::Redirect::Inherit, error: Process::Redirect::Inherit)
      status = process.wait
      if status.success?
        puts "Compilation completed successfully.".colorize(:green)
        return true
      else
        puts "Compilation failed with exit code #{status.exit_code}".colorize(:red)
        return false
      end
    rescue error
      puts "Error executing command: #{error.message}".colorize(:red)
  ...
  def self.help_command(show_banner : Bool)
    if show_banner
      puts "Hacker Lang CLI - Simplified Scripting Tool v#{VERSION}".colorize.mode(:bold).back(:magenta).fore(:white)
    end

    puts "Available Commands:".colorize(:blue).mode(:bold)
    ...
    puts "\nGlobal options:".colorize(:light_gray).mode(:bold)
  ...
  def self.version_command
    puts "Hacker Lang CLI v#{VERSION}".colorize(:cyan)
    true
  end
    ...
    when "run"
      ...
      OptionParser.parse do |parser|
        ...
        parser.unknown_args do |args|
          if args.size != 1
            puts "Error: Expected exactly one argument: <file>".colorize(:red)
            parser.banner
            exit(1)
          end
          file = args[0]
